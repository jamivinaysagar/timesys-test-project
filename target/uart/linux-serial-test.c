...
void display_help()
{
	printf("Usage: linux-serial-test [OPTION]\n"
			...
			"  -w, --tx-bytes    Number of bytes for each write (default is to repeatedly write 4096 bytes\n"
			...
			"  -L, --hw-loop     Enable hardware loopback\n"
			"\n"
	      );
	exit(0);
}
...
void process_options(int argc, char * argv[])
{
	for (;;) {
		int option_index = 0;
		static const char *short_options = "hb:p:d:R:TsSy:z:certq:l:a:w:o:i:L";
...
int main(int argc, char * argv[])
{
	...
	_write_size = (_cl_tx_bytes == 0) ? 4096 : _cl_tx_bytes;
